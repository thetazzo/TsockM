...
fn request_connection(addr: net.Address) !net.Stream {
...
fn read_cmd(addr: net.Address, sid: usize) !void {
    while (true) {
        // read for command
...
pub fn start() !void {
    ...
    const comm_stream = try request_connection(addr);

    const t1 = try std.Thread.spawn(.{}, listen_for_comms, .{comm_stream});
    const t2 = try std.Thread.spawn(.{}, read_cmd, .{ addr, 0 });
    t1.join();
    t2.join();
