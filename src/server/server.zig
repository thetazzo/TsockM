...
fn peer_kill(
    ...
    if (peer_ref) |pf| {
        ...
        _ = ptc.transmit("RESPONSE", peer_pool.items[pf.i].stream, endp, LOG_LEVEL);
    ...
    if (peer_ref) |pf| {
        for (peer_pool.items[0..]) |peer| {
            if (pf.i != pind and peer.alive) {
                ...
                _ = ptc.transmit("RESPONSE", peer.conn.stream, msgp, LOG_LEVEL);
            }
            pind += 1;
        }
    }
...
fn read_incomming(
    ...
    if (protocol.is_request()) {
        if (protocol.is_action(ptc.Act.COMM)) {
            ...
            ptc.transmit("RESPONSE", stream, resp, LOG_LEVEL).dump("RESPONSE", LOG_LEVEL);
            ...
            _ = ptc.transmit("RESPONSE", stream, errp, LOG_LEVEL);
        }
